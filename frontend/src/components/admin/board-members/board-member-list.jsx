...
function Portrait({ item, className }) {
  return (
    <div className={`overflow-hidden rounded-md bg-surface ${className || ""}`}>
          ...
          <TableBody>
            {items.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell className="font-semibold text-ink">{item.fullName}</TableCell>
                <TableCell>
                  <div className="grid gap-1">
                    <span className="text-sm font-medium text-ink">{item.titleTr}</span>
                    <span className="text-xs text-muted">{item.titleEn}</span>
                  </div>
                </TableCell>
        ...
        {items.map((item) => (
          <article className="grid grid-cols-[76px_minmax(0,1fr)] gap-4 rounded-lg border border-line bg-white p-4" key={item.id}>
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted">{item.titleTr}</p>
            </div>
