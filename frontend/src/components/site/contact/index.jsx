...
export async function ContactPageContent({ locale }) {
  ...
  const structuredData = {
    ...
    "@type": "ContactPage",
    name: "Tüketici Birliği İletişim",
      ...
      <div>
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {channels.map((item) => (
            <StaticCard key={item.label}>
              ...
              <h2 className="mt-4 text-sm font-bold uppercase tracking-[0.08em] text-muted">{item.label}</h2>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_1fr]">
          <StaticCard>
            ...
            <p className="mt-2 text-sm leading-6 text-muted">Genel soru, öneri ve iletişim talepleri için kısa form.</p>
            <div className="mt-6">
              <ContactForm />
            </div>
          </StaticCard>
          ...
          <StaticCard className="overflow-hidden p-0">
            <iframe className="h-[520px] w-full border-0" loading="lazy" referrerPolicy="no-referrer-when-downgrade" src={mapUrl} title="Tüketici Birliği harita konumu" />
          </StaticCard>
        </div>
      </div>
    </section>
