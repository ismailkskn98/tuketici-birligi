    ...
    <section className="grid content-start gap-5">
      <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-3 font-heading text-3xl font-semibold tracking-tight text-ink md:text-[2.4rem]">{title}</h2>
          {description ? <p className="mt-3 text-sm leading-7 text-muted md:max-w-xl">{description}</p> : null}
        </div>

        <Link
          className="focus-ring inline-flex shrink-0 items-center gap-1.5 text-sm font-semibold text-ink/72 transition hover:text-secondary-dark"
          href={viewAllHref}
        >
          {viewAllLabel}
