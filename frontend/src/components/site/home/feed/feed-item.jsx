    ...
    <div className={hasCardLayout ? "overflow-hidden rounded-[24px] border border-line/80 bg-white shadow-[0_16px_38px_rgba(22,32,51,0.06)]" : ""}>
      ...
      <div className={hasCardLayout ? "p-5" : "flex items-start justify-between gap-4"}>
        <div className="min-w-0">
          {categoryLabel ? (
            <Badge className="rounded-full bg-primary-soft px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-primary-dark">
              {categoryLabel}
            </Badge>
          ) : null}
          {date ? (
            <time
              className="mt-3 block text-[11px] font-semibold uppercase tracking-[0.14em] text-muted"
              dateTime={date}
            >
        ...
        {!hasCardLayout ? (
          <ArrowUpRight
            aria-hidden="true"
            className="mt-1 size-4 shrink-0 text-ink/30 transition duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-secondary"
          />
        ) : null}
      </div>
      ...
      {hasCardLayout ? (
        <div className="flex items-center justify-between border-t border-line/80 px-5 py-4 text-sm font-semibold text-ink/75 transition group-hover:text-secondary-dark">
          ...
          <ArrowUpRight
            aria-hidden="true"
            className="size-4 shrink-0 transition duration-200 group-hover:-translate-y-0.5 group-hover:translate-x-0.5"
          />
        </div>
      ) : null}
    </div>
