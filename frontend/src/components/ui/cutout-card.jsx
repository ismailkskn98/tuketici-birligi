...
export const cutoutCardSurfaceClassName = cn("group/cutout relative cursor-pointer overflow-hidden rounded-[28px] bg-card text-card-foreground", cutoutCardSurfaceShadowClassName);
...
export function CutoutCard({ className, hovered: hoveredProp, defaultHovered = false, onHoveredChange, trackPointerHover = true, onMouseEnter, onMouseLeave, children, ...props }) {
  const reduceMotion = useReducedMotion();
...
export function CutoutCardImage({ className, alt = "", fill = true, sizes = "(max-width: 768px) 100vw, 28rem", ...props }) {
  return (
    <Image
      alt={alt}
...
export function CutoutCardOverlay({ className, ...props }) {
  return (
    <div
      className={cn("pointer-events-none absolute inset-0 bg-linear-to-t from-background/35 via-transparent to-transparent dark:from-background/50", className)}
...
export function CutoutCorner({ className, size = 32, viewBox = "0 0 200 200", ...props }) {
  return (
    <>
