...
const MODES = ["uniform", "multi", "wave", "random"];
const PREFIX_FONTS = ["none", "square", "grid", "circle", "triangle", "line"];
...
export default function PixelHeadingCharacterDemo() {
  ...
  const [prefixFont, setPrefixFont] = useState("none");
  ...
  const isolateMap = isolateEnabled ? Object.fromEntries(isolateChars.split("").map((c) => [c, isolateFont])) : undefined;
    ...
    <div className="w-full space-y-8 py-4">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {MODES.map((m) => (
              <button
                ...
                className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${mode === m ? "bg-foreground text-background" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
              >
                {m}
              </button>
            ))}
        ...
        <ControlGroup label={`Cycle Interval: ${cycleInterval}ms`}>
          <input type="range" min={30} max={500} step={10} value={cycleInterval} onChange={(e) => setCycleInterval(Number(e.target.value))} className="w-full accent-foreground" />
        </ControlGroup>

        {/* Stagger Delay */}
        <ControlGroup label={`Stagger Delay: ${staggerDelay}ms`}>
          <input type="range" min={0} max={200} step={5} value={staggerDelay} onChange={(e) => setStaggerDelay(Number(e.target.value))} className="w-full accent-foreground" />
        </ControlGroup>

        {/* Default Font Index (uniform) */}
        <ControlGroup label={`Default Font Index: ${defaultFontIndex}`}>
          <input type="range" min={0} max={4} step={1} value={defaultFontIndex} onChange={(e) => setDefaultFontIndex(Number(e.target.value))} className="w-full accent-foreground" />
        </ControlGroup>
        ...
        <ControlGroup label="Prefix Font">
          <select
            value={prefixFont}
            onChange={(e) => setPrefixFont(e.target.value)}
            className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
          >
            {PREFIX_FONTS.map((f) => (
...
function Toggle({ checked, onChange }) {
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors ${checked ? "bg-foreground" : "bg-input"}`}
    >
